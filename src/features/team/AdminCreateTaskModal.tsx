import { useEffect, useState } from 'react'
import { Plus, X, Calendar } from 'lucide-react'
import { supabase } from '@/lib/supabase'
import { useAuthStore } from '@/store/auth.store'
import { useUIStore } from '@/store/ui.store'
import { DropdownMenu } from '@/components/ui/DropdownMenu'
import { CalendarModal } from '@/components/ui/CalendarModal'

interface EventOption {
  id: string
  name: string
}

interface PhaseOption {
  id: string
  phase_number: number
  phase_name: string
}

interface ProfileOption {
  id: string
  display_name: string | null
  email: string
}

interface AdminCreateTaskModalProps {
  onCreated: () => void
  onCancel: () => void
}

const PRIORITIES = ['low', 'medium', 'high', 'urgent'] as const
const PRIORITY_LABELS: Record<string, string> = {
  low: 'Low', medium: 'Medium', high: 'High', urgent: 'Urgent',
}

export function AdminCreateTaskModal({ onCreated, onCancel }: AdminCreateTaskModalProps) {
  const user = useAuthStore((s) => s.user)
  const showNotification = useUIStore((s) => s.showNotification)
  const [saving, setSaving] = useState(false)
  const [calendarOpen, setCalendarOpen] = useState(false)
  const [events, setEvents] = useState<EventOption[]>([])
  const [phases, setPhases] = useState<PhaseOption[]>([])
  const [profiles, setProfiles] = useState<ProfileOption[]>([])

  const [form, setForm] = useState({
    event_id: '',
    title: '',
    description: '',
    phase_id: '',
    assignee_id: '',
    due_datetime: '',
    priority: 'medium',
  })

  useEffect(() => {
    loadEvents()
    loadProfiles()
  }, [])

  async function loadEvents() {
    const { data } = await supabase
      .from('events')
      .select('id, name')
      .is('deleted_at', null)
      .order('name', { ascending: true })
    if (data) setEvents(data as unknown as EventOption[])
  }

  async function loadProfiles() {
    const { data } = await supabase
      .from('profiles')
      .select('id, display_name, email')
      .or('is_super_admin.eq.true,role.eq.team_member')
      .order('display_name', { ascending: true, nullsFirst: false })
    if (data) setProfiles(data as unknown as ProfileOption[])
  }

  async function loadPhases(eventId: string) {
    const { data } = await supabase
      .from('event_phases')
      .select('id, phase_number, phase_name')
      .eq('event_id', eventId)
      .order('phase_number', { ascending: true })
    if (data) setPhases(data as unknown as PhaseOption[])
  }

  function handleEventSelect(value: string) {
    setForm({ ...form, event_id: value, phase_id: '' })
    if (value) loadPhases(value)
    else setPhases([])
  }

  async function handleSave() {
    if (!form.event_id) {
      showNotification({ variant: 'warning', title: 'Missing field', message: 'Please select an event' })
      return
    }
    if (!form.title.trim()) {
      showNotification({ variant: 'warning', title: 'Missing field', message: 'Title is required' })
      return
    }
    if (!user) return

    setSaving(true)

    const { error } = await supabase
      .from('tasks')
      .insert({
        event_id: form.event_id,
        title: form.title.trim(),
        description: form.description.trim() || null,
        assignee_id: form.assignee_id || null,
        phase_id: form.phase_id || null,
        created_by: user.id,
        due_datetime: form.due_datetime || null,
        priority: form.priority,
        status: 'pending',
      })

    if (error) {
      showNotification({ variant: 'error', title: 'Failed to create task', message: error.message })
      setSaving(false)
      return
    }

    showNotification({ variant: 'success', title: 'Task created' })
    setSaving(false)
    onCreated()
  }

  const selectedEvent = events.find((e) => e.id === form.event_id)
  const sortedPhases = [...phases].sort((a, b) => a.phase_number - b.phase_number)

  return (
    <div className="overlay" onClick={onCancel}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        <div className="modal-card__header">
          <h3 className="modal-card__title">Create Task</h3>
          <button className="modal-card__close" onClick={onCancel} aria-label="Close">
            <X size={18} />
          </button>
        </div>
        <div className="modal-card__body">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            {/* Event */}
            <div className="input-wrapper">
              <label className="input-label">Event <span className="required">*</span></label>
              <DropdownMenu
                trigger={
                  <span style={{ color: form.event_id ? 'var(--color-text-primary)' : 'var(--color-text-muted)' }}>
                    {selectedEvent?.name || 'Select event'}
                  </span>
                }
                items={events.map((e) => ({ label: e.name, value: e.id }))}
                onSelect={(item) => handleEventSelect(item.value)}
              />
            </div>

            {/* Title */}
            <div className="input-wrapper">
              <label className="input-label">Title <span className="required">*</span></label>
              <input
                className="input"
                placeholder="What needs to be done?"
                value={form.title}
                onChange={(e) => setForm({ ...form, title: e.target.value })}
                onKeyDown={(e) => e.key === 'Enter' && handleSave()}
                autoFocus
              />
            </div>

            {/* Description */}
            <div className="input-wrapper">
              <label className="input-label">Description</label>
              <textarea
                className="input"
                style={{ minHeight: 80, resize: 'vertical' }}
                placeholder="Task details..."
                value={form.description}
                onChange={(e) => setForm({ ...form, description: e.target.value })}
              />
            </div>

            {/* Phase */}
            <div className="input-wrapper">
              <label className="input-label">Phase</label>
              <DropdownMenu
                trigger={
                  <span style={{ color: form.phase_id ? 'var(--color-text-primary)' : 'var(--color-text-muted)' }}>
                    {form.phase_id
                      ? sortedPhases.find((p) => p.id === form.phase_id)?.phase_name || 'Unknown phase'
                      : form.event_id ? 'No phase' : 'Select an event first'}
                  </span>
                }
                items={[
                  { label: 'No phase', value: '' },
                  ...sortedPhases.map((p) => ({
                    label: `Phase ${p.phase_number}: ${p.phase_name}`,
                    value: p.id,
                  })),
                ]}
                onSelect={(item) => setForm({ ...form, phase_id: item.value })}
                disabled={!form.event_id}
              />
            </div>

            {/* Assignee */}
            <div className="input-wrapper">
              <label className="input-label">Assignee</label>
              <DropdownMenu
                trigger={
                  <span style={{ color: form.assignee_id ? 'var(--color-text-primary)' : 'var(--color-text-muted)' }}>
                    {form.assignee_id
                      ? profiles.find((p) => p.id === form.assignee_id)?.display_name
                        || profiles.find((p) => p.id === form.assignee_id)?.email
                        || 'Select member'
                      : 'Unassigned'}
                  </span>
                }
                items={[
                  { label: 'Unassigned', value: '' },
                  ...profiles.map((p) => ({
                    label: p.display_name || p.email,
                    value: p.id,
                  })),
                ]}
                onSelect={(item) => setForm({ ...form, assignee_id: item.value })}
              />
            </div>

            {/* Due + Priority */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
              <div className="input-wrapper">
                <label className="input-label">Due Date</label>
                <button
                  className="input"
                  type="button"
                  style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', textAlign: 'left', cursor: 'pointer' }}
                  onClick={() => setCalendarOpen(true)}
                >
                  <Calendar size={14} style={{ color: 'var(--color-text-muted)', flexShrink: 0 }} />
                  <span style={{ color: form.due_datetime ? 'var(--color-text-primary)' : 'var(--color-text-muted)' }}>
                    {form.due_datetime
                      ? new Date(form.due_datetime.split('T')[0]).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
                      : 'Select date'}
                  </span>
                </button>
              </div>

              <div className="input-wrapper">
                <label className="input-label">Priority</label>
                <DropdownMenu
                  trigger={<span style={{ color: 'var(--color-text-primary)' }}>{PRIORITY_LABELS[form.priority]}</span>}
                  items={PRIORITIES.map((p) => ({ label: PRIORITY_LABELS[p], value: p }))}
                  onSelect={(item) => setForm({ ...form, priority: item.value })}
                />
              </div>
            </div>
          </div>

          <div style={{ display: 'flex', gap: 'var(--space-2)', marginTop: 'var(--space-4)' }}>
            <button className="btn btn-primary btn-sm" onClick={handleSave} disabled={saving}>
              <Plus size={14} />
              {saving ? 'Creating...' : 'Create Task'}
            </button>
            <button className="btn btn-ghost btn-sm" onClick={onCancel}>Cancel</button>
          </div>
        </div>

        <CalendarModal
          open={calendarOpen}
          value={form.due_datetime ? form.due_datetime.split('T')[0] : ''}
          onChange={(d) => setForm({ ...form, due_datetime: `${d}T23:59:00` })}
          onClose={() => setCalendarOpen(false)}
        />
      </div>
    </div>
  )
}
